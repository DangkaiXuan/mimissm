...
public class ProductInfoAction {
    ...
    public String save(ProductInfo info, HttpServletRequest request) {
        ...
        //info对象中的表单提交上来的5个数据,有异步ajax上来的图片名称数据,有上架时间数据
        int num = -1;
        try {
        ...
        //清空saveFileName变量中内容，,为了下次增加或修改的异步ajax的上传处理
        saveFileName = "";
    ...
    public String one(int pid,ProductInfoVo vo, Model model,HttpSession session) {
        ...
        //将多条件及页码放入session,更新处理结束分页时读取条件和页码进行处理
    ...
    public String update(ProductInfo info, HttpServletRequest request) {
        //因为ajax的异步图片上传,
        // 则saveFileName里有上传过来的图片如果没有异步ajax上传过，
        // 则saveFileName里面内容="",实体类info使用隐藏表单域提供上来的pImage原始图片的名称
        ...
        // 而下一次更新时要使用这个变量作为判断的一句,就会出错,所以必须清空saveFileName
        saveFileName = "";
        //重定向跳转
        return "forward:/prod/split.action";
    }
